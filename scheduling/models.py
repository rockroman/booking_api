...
class TimeSlot(models.Model):
    service = models.ForeignKey(Service, on_delete=models.CASCADE, related_name='timeslots')
    ...
    def clean(self):
        ...
        # the service duration time and not exceeding it
            if self.service:
                ...
                self.end_time = (datetime.combine(date(1, 1, 1), self.start_time) + service_duration).time()
            else:
                raise ValidationError("Service must be specified to calculate end_time.")
        ...
        if self.start_time is not None and self.end_time is not None:
            ...
            if overlapping_time_slots.exists():
                raise ValidationError("This time slot overlaps with another time slot for the same service.")
    ...
    #     #     raise ValidationError("Time slot duration cannot exceed service duration.")
